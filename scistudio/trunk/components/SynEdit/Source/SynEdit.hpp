...
enum TSynSearchOption { ssoMatchCase, ssoWholeWord, ssoBackwards, ssoEntireScope, ssoSelectedOnly, ssoReplace, 
...
enum TSynStateFlag { sfCaretChanged, sfScrollbarChanged, sfLinesChanging, sfIgnoreNextChar, sfCaretVisible, 
	sfDblClicked, sfPossibleGutterClick, sfWaitForDragging, sfInsideRedo };
#pragma option pop
...
typedef void __fastcall (__closure *TGutterClickEvent)(System::TObject* Sender, int X, int Y, int Line
	, TSynEditMark* mark);
...
private:
	...
	void __fastcall SetCaretAndSelection(const Windows::TPoint &ptCaret, const Windows::TPoint &ptBefore
		, const Windows::TPoint &ptAfter);
...
protected:
	...
	void __fastcall ListAdded(int Index);
...
public:
	...
	Windows::TPoint __fastcall LogicalToPhysicalPos(const Windows::TPoint &p);
	...
	Windows::TPoint __fastcall RowColumnToPixels(const Windows::TPoint &RowCol);
	int __fastcall SearchReplace(const AnsiString ASearch, const AnsiString AReplace, TSynSearchOptions 
		AOptions);
